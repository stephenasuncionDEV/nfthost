...
export const useAnalytics = () => {
    ...
    const getWebsiteVisitsByRoute = async () => {
        try {
            ...
            const res = await axios.get('https://app.posthog.com/api/projects/9428/insights/849818', {
                headers: {
            ...
            const insights = result.filter((result) => routesArr.includes(result.breakdown_value)) || [];

            const websiteVisits = insights.map((insight) => {
                return {
                    name: insight.breakdown_value,
                    visits: insight.aggregated_value,
                    fill: websiteColors.find((web) => web.name === insight.breakdown_value).color
                }
            });
    ...
    const getWebsiteUniqueVisitsByRoute = async () => {
        try {
            ...
            const res = await axios.get('https://app.posthog.com/api/projects/9428/insights/850010', {
                headers: {
                    Authorization: `Bearer ${process.env.POSTHOG_PERSONAL_KEY}`
                }
            })
