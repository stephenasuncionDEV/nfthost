...
import {
    XAxis, YAxis, Tooltip, Legend, BarChart, LabelList, Bar,
...
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        return (
            <Flex flexDir='column' justifyContent='center' alignItems='center' w='200px' p='.25em'>
                <Text>
                    {label}: <span style={{ color: 'rgb(117,63,229)', fontWeight: 'bold' }}>{payload[0].value}</span>
                </Text>
            </Flex>
        );
    }
...
const renderActiveShape = (props) => {
    ...
    return (
        <g>
            <text x={cx} y={cy} dy={8} textAnchor="middle" fill={fill} fontSize="10pt">
                {payload.name}
            </text>
            <Sector
                cx={cx}
            ...
            <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill="#fff" fontSize="9pt">
                Unique visits: {value}
            </text>
            <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} dy={18} textAnchor={textAnchor} fill="#fff" fontSize="8pt">
